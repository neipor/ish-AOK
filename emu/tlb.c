...
void tlb_flush(struct tlb *tlb) {
    static const struct tlb_entry invalid = {.page = TLB_PAGE_EMPTY, .page_if_writable = TLB_PAGE_EMPTY};
    tlb->mem_changes = tlb->mmu->changes;
    for (unsigned i = 0; i < TLB_SIZE; i++)
        tlb->entries[i] = invalid;
}
...
__no_instrument void *tlb_handle_miss(struct tlb *tlb, addr_t addr, int type) {
    ...
    char *ptr = mmu_translate(tlb->mmu, page, type);
    ...
    tlb_ent->data_minus_addr = (uintptr_t) ptr - page;
